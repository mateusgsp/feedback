...
export function FeedbackSuccessStep({ onFeedbackRestartRequested }: FeedbackSuccessStepProps) {
  return (
    <>
      <header className="w-full p-4 flex border-b border-gray-200 h-14">
        Agradecemos o feedback
        <CloseButton />
      </header>

      <div className="flex flex-col items-center pb-4 pt-4 w-[304px]">
        ...
        <span className="text-xl mt-2 mx-4">Eviado com sucesso</span>
      </div>
      <footer className="flex gap-2 mt-2 bg-gray-200 w-full p-4 rounded-b-2xl">
        <button
          type="button"
          onClick={onFeedbackRestartRequested}
          className="w-full py-2 px-6 bg-zinc-300 rounded-md border-transparent text-sm leading-6 hover:bg-zinc-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-brand-500 transition-colors"
        >
          Quero enviar outro
        </button>
      </footer>
